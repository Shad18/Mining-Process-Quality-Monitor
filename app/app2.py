"""
Industrial Quality Monitoring Platform
...
def _dash_run_models(df, models):
    """
    Run all three models on the latest available row and return a dict of results.
    Kept outside page_dashboard so the logic is easy to unit-test.
    Returns:
        pred_now   – silica prediction from model_with_iron  (float | None)
        pred_1h    – silica prediction from model_1h_ahead   (float | None)
        confidence – 0-100 float derived from prediction residuals
        residuals  – pd.Series of |predicted - actual| over last 48 rows
        model_ok   – bool
        pred_time  – datetime when prediction was made
    """
    results = dict(pred_now=None, pred_1h=None, confidence=None,
                   residuals=None, model_ok=False, pred_time=None)
    try:
        latest_row = df.iloc[-1].to_dict()

        # ── current-time prediction (model_with_iron) ──────────────────────
        m_now   = models["with_iron"]
        f_now   = get_model_features(m_now)
        X_now   = pd.DataFrame([{f: float(latest_row.get(f, 0)) for f in f_now}])
        pred_now = float(m_now.predict(X_now)[0])

        # ── 1-hour ahead prediction ─────────────────────────────────────────
        m_1h  = models["1h_ahead"]
        f_1h  = get_model_features(m_1h)
        X_1h  = pd.DataFrame([{f: float(latest_row.get(f, 0)) for f in f_1h}])
        pred_1h = float(m_1h.predict(X_1h)[0])

        # ── confidence via residuals on last 48 rows ────────────────────────
        # Predict on last 48 rows where we *know* the actual silica value,
        # compute |error| per row, then map RMSE → 0-100 confidence score.
        tail48 = df.tail(48).copy()
        avail  = [f for f in f_now if f in tail48.columns]
        miss   = [f for f in f_now if f not in tail48.columns]
        for mf in miss:
            tail48[mf] = 0.0
        X_tail = tail48[f_now].fillna(0)
        preds_tail = m_now.predict(X_tail)
        actual_tail = tail48['% Silica Concentrate'].values
        residuals   = pd.Series(np.abs(preds_tail - actual_tail))
        rmse = float(np.sqrt(np.mean(residuals ** 2)))
        # Sigmoid-style mapping: RMSE=0 → 100%, RMSE=1 → ~37%, RMSE≥2 → <15%
        confidence = round(100 * np.exp(-rmse), 1)
        confidence = max(5.0, min(99.9, confidence))

        results.update(
            pred_now=pred_now, pred_1h=pred_1h,
            confidence=confidence, residuals=residuals,
            model_ok=True, pred_time=datetime.now(),
        )
    except Exception:
        pass
    return results
...
def page_dashboard():
    ...
    latest     = df.iloc[-1]
    silica_out = latest.get('% Silica Concentrate', np.nan)   # actual sensor reading
    ...
    # ── Run models ──────────────────────────────────────────────────────────
    dash = _dash_run_models(df, models)
    pred_now   = dash["pred_now"]
    pred_1h    = dash["pred_1h"]
    confidence = dash["confidence"]
    residuals  = dash["residuals"]
    model_ok   = dash["model_ok"]
    pred_time  = dash["pred_time"]

    # Use model prediction as the primary silica display when available
    display_silica = pred_now if (model_ok and pred_now is not None) else silica_out
    risk, rcol, _  = silica_risk_level(display_silica)

    # ── Process Stability — based on prediction residuals ───────────────────
    # Definition: average absolute error over last 48 predictions mapped to 0-100.
    # Low residuals → stable process; high residuals → process drifting.
    if model_ok and residuals is not None and len(residuals) > 0:
        mean_abs_err = float(residuals.mean())
        # Scale: MAE=0 → 100%, MAE=0.5 → ~37%, MAE≥1 → <14%
        stability = round(100 * np.exp(-mean_abs_err * 2), 1)
        stability = max(0.0, min(100.0, stability))
        stability_sub = f"MAE={mean_abs_err:.3f} over last 48 h"
    else:
        # Fallback to sensor std if models failed
        std_24 = df['% Silica Concentrate'].tail(24).std()
        stability = round(max(0, min(100, 100 - std_24 * 20)), 1)
        stability_sub = "Sensor std-dev fallback"

    stab_col = ACCENT_GREEN if stability >= 70 else (ACCENT_AMBER if stability >= 40 else ACCENT_RED)

    # ── Model status banner ──────────────────────────────────────────────────
    if model_ok and pred_time:
        elapsed_s  = int((datetime.now() - pred_time).total_seconds())
        elapsed_lbl = f"{elapsed_s}s ago" if elapsed_s < 60 else f"{elapsed_s//60}m ago"
        conf_col   = ACCENT_GREEN if confidence >= 70 else (ACCENT_AMBER if confidence >= 45 else ACCENT_RED)
        conf_bar   = int(confidence)
        st.markdown(f"""
        <div style="background:{CARD_BG}; border:1px solid {CARD_BORDER};
             border-left:4px solid {ACCENT_GREEN}; border-radius:10px;
             padding:10px 18px; margin-bottom:16px;
             display:flex; align-items:center; gap:20px; flex-wrap:wrap;">
            <div>
                <span style="font-size:0.72rem;color:{TEXT_MUTED};
                      text-transform:uppercase;letter-spacing:.07em;">Model Status</span><br>
                <span style="color:{ACCENT_GREEN};font-weight:600;font-size:0.9rem;">
                    ● ACTIVE — Prediction updated {elapsed_lbl}
                </span>
            </div>
            <div style="flex:1;min-width:180px;">
                ...
                <div style="background:{CARD_BORDER};border-radius:4px;
                     height:6px;margin-top:5px;overflow:hidden;">
                    <div style="background:{conf_col};width:{conf_bar}%;height:100%;
                         border-radius:4px;transition:width 0.4s;"></div>
                </div>
            </div>
            <div style="font-size:0.8rem;color:{TEXT_MUTED};">
                XGBoost (with_iron) · 1h-ahead active
            </div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div style="background:{CARD_BG}; border:1px solid {ACCENT_AMBER};
             border-left:4px solid {ACCENT_AMBER}; border-radius:10px;
             padding:10px 18px; margin-bottom:16px;">
            <span style="color:{ACCENT_AMBER};font-weight:600;">
                ⚠️ Model inference unavailable — showing sensor values only
            </span>
        </div>
        """, unsafe_allow_html=True)

    # ── ROW 1: model-driven KPI cards ───────────────────────────────────────
    cols = st.columns(4)
    with cols[0]:
        card("Iron Feed",  f"{iron_feed:.1f}%",  "% Fe in raw ore feed",  ACCENT_CYAN,  "🪨")
    with cols[1]:
        # Silica prediction from XGBoost, not raw sensor
        pred_sub = (f"XGBoost prediction · actual {silica_out:.2f}%"
                    if model_ok else "Sensor reading (model unavailable)")
        card("Silica Predict",  f"{display_silica:.2f}%", pred_sub, rcol, "🔬")
    with cols[2]:
        # 1-Hour Forecast
        if model_ok and pred_1h is not None:
            fcast_risk, fcast_col, _ = silica_risk_level(pred_1h)
            delta_1h = pred_1h - display_silica
            arrow    = "↑" if delta_1h > 0 else "↓"
            fcast_sub = f"{arrow}{abs(delta_1h):.2f}% vs now · risk {fcast_risk}"
            card("1-Hour Forecast", f"{pred_1h:.2f}%", fcast_sub, fcast_col, "📡")
        else:
            card("1-Hour Forecast", "—", "Model unavailable", TEXT_MUTED, "📡")
    with cols[3]:
        # Confidence card
        if model_ok:
            conf_lbl = "HIGH" if confidence >= 70 else ("MEDIUM" if confidence >= 45 else "LOW")
            conf_col2 = ACCENT_GREEN if confidence >= 70 else (ACCENT_AMBER if confidence >= 45 else ACCENT_RED)
            card("Pred. Confidence", f"{confidence:.1f}%",
                 f"Based on 48-row residuals · {conf_lbl}", conf_col2, "📊")
        else:
            card("Pred. Confidence", "—", "Model unavailable", TEXT_MUTED, "📊")

    # ── ROW 2: live sensor cards ─────────────────────────────────────────────
    cols2 = st.columns(4)
    with cols2[0]:
        card("Starch Flow", f"{latest.get('Starch Flow', 0):.1f}", "m³/h",
             ACCENT_CYAN, "💧")
    with cols2[1]:
        card("Ore Pulp pH", f"{latest.get('Ore Pulp pH', 0):.2f}", "pH",
             ACCENT_AMBER, "⚗️")
    with cols2[2]:
        card("Ore Flow", f"{latest.get('Ore Pulp Flow', 0):.0f}", "t/h",
             ACCENT_GREEN, "⛏️")
    with cols2[3]:
    ...
    with c1:
        ...
        fig.add_trace(go.Scatter(
            x=tail['date'], y=tail['% Silica Concentrate'],
            mode='lines', fill='tozeroy',
            fillcolor='rgba(57,208,245,0.12)',
            line=dict(color=ACCENT_CYAN, width=2),
            name='Actual Silica'))
        # Overlay model predictions on the same 72-row window
        if model_ok:
            m_now  = models["with_iron"]
            f_now  = get_model_features(m_now)
            miss   = [f for f in f_now if f not in tail.columns]
            tail_p = tail.copy()
            for mf in miss:
                tail_p[mf] = 0.0
            X_tail72 = tail_p[f_now].fillna(0)
            p72 = m_now.predict(X_tail72)
            fig.add_trace(go.Scatter(
                x=tail['date'], y=p72,
                mode='lines',
        ...
        fig.update_layout(title="Silica Concentrate – Last 72 h (Actual + Model)",
    ...
    with c2:
        ...
        fig2.add_trace(go.Scatter(
            x=tail['date'], y=tail['% Iron Concentrate'],
            mode='lines', fill='tozeroy',
    ...
    c3, c4 = st.columns(2)
    with c3:
        # Gauge shows MODEL prediction, delta vs actual sensor
        gauge_val   = display_silica
        gauge_delta = (display_silica - silica_out) if model_ok else 0.0
        fig_g = go.Figure(go.Indicator(
            mode="gauge+number+delta",
            value=float(gauge_val),
            title={'text': "Predicted Silica (%)", 'font': {'color': TEXT_PRIMARY}},
            delta={
                'reference': float(silica_out),
    ...
    with c4:
        # Process Stability gauge — residual-based
        fig_stab = go.Figure(go.Indicator(
            mode="gauge+number",
            value=stability,
            number={'suffix': '%', 'font': {'color': stab_col}},
            title={'text': "Process Stability", 'font': {'color': TEXT_PRIMARY}},
            gauge={
                'axis': {'range': [0, 100], 'tickcolor': TEXT_MUTED,
                         'tickvals': [0, 25, 50, 70, 85, 100]},
                'bar': {'color': stab_col},
                'steps': [
                    {'range': [0,  40],  'color': 'rgba(248,81,73,0.12)'},
                    {'range': [40, 70],  'color': 'rgba(210,153,34,0.12)'},
                    {'range': [70, 100], 'color': 'rgba(63,185,80,0.12)'},
                ],
                'threshold': {
        ...
        apply_template(fig_stab)
        st.plotly_chart(fig_stab, use_container_width=True)

    # ── Residual error sparkline (only when models ran) ──────────────────────
    if model_ok and residuals is not None:
        st.markdown('<div class="section-title">Prediction Residuals – Last 48 h '
                    '(|Predicted − Actual|)</div>', unsafe_allow_html=True)
        res_df = pd.DataFrame({
            'Hour': range(len(residuals)),
            'Abs Error': residuals.values,
        })
        fig_res = go.Figure()
        fig_res.add_trace(go.Bar(
            x=res_df['Hour'], y=res_df['Abs Error'],
            marker_color=[
                ACCENT_RED if v > 0.5 else (ACCENT_AMBER if v > 0.25 else ACCENT_GREEN)
                for v in res_df['Abs Error']
            ],
            name='|Error|',
            hovertemplate='Hour %{x}<br>Abs Error: %{y:.3f}%<extra></extra>',
        ))
        fig_res.add_hline(y=float(residuals.mean()), line_dash='dash',
                          line_color=TEXT_MUTED,
                          annotation_text=f"Mean {residuals.mean():.3f}%")
        fig_res.update_layout(
            height=200,
            margin=dict(l=0, r=0, t=10, b=0),
            showlegend=False,
        )
        apply_template(fig_res)
        st.plotly_chart(fig_res, use_container_width=True)

    # ── Air flow donut + heatmap ──────────────────────────────────────────────
    ...
    with c5:
        ...
        fig_d.update_layout(title="Air Flow by Column",
                            height=300, margin=dict(l=0, r=0, t=40, b=0))
    ...
    with c6:
        ...
        hm_piv = hm.pivot(index='day_of_week', columns='hour',
                           values='% Silica Concentrate')
...
<body>
<h1>🏭 Industrial Quality Monitoring Report</h1>
...
<div class="footer">Industrial Quality Monitoring Platform &copy; {datetime.now().year}</div>
</body>
</html>"""
